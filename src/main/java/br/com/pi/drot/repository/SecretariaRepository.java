...
public class SecretariaRepository implements SecretariaDAO{
	...
	public boolean logado(String email, String pass) {
		try {
			Secretaria s = this.connection.getEntityManager().createNamedQuery("Secretaria.loginSecretaria", Secretaria.class).setParameter("email", email).setParameter("senha", pass).getSingleResult();
			return true;
		} catch (NoResultException e) {
			return false;
		}
